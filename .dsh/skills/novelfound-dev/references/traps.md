...
> **自检本身的可靠性铁律见 `SKILL.md` §2**（断言只盯用户可见结果、反向对照、
> 探针护栏、几何护栏、假绿防范）。下面这些是自检特有的坑。

- 它**联网**：搜索 → 详情 → 阅读全流程，跑一次约 1 分钟；别和重活并行，也别反复空跑。
- 每次运行会 `rmtree` 独立数据目录 `tests/.uicheck`（不影响用户真实书架）。
- 会写 21 张截图到 `tests/shots/`，**这些图现在是可信的**（图里有文字），
...
- **像素判据要抓稳定帧**：`grab()` 可能抓到上一帧 → 误报"贴底/裁切"。
  连续两次抓图内容一致再用（`verify_ui.py` 的 `stable_grab`）。
- **别用 `document().size().height()` 判断"有没有溢出"**：它和分页用的是同一个高度模型，
  属于循环验证（曾经 259 项全绿而界面在裁字）。裁切只认像素。
- **偶发失败的典型来源是网络内容**，不是代码：
...
- **生成测试图片不要手写十六进制字节**：拼错的 PNG 能过"字节头"断言、解码时才失败
  （`IDAT: incorrect data check`）。用 `zlib` + CRC 现场生成，并加一条 CRC 单测。
